...
1. 程序的并发执行
   ...
   - 2.程序的并发执行
     - 含义：程序并发执行是指在同一时间间隔内运行多个程序，一个程序执行结束之前，可以运行其他程序。
...
3. 进程控制块（PCB）
   ...
   - 1.什么是进程控制块
     ...
     - 进程控制块是进程实体的一部分，是操作系统中最重要的`数据结构`。进程控制块中记录了操作系统所需要的、用于描述进程情况及控制进程运行所需的全部信息。
       - 每个进程有唯一 PCB
       - PCB 是进程存在的唯一标志
       - OS 通过 PCB 控制和管理进程
       - OS 通过 PCB 感知进程的存在
...
1. 进程的创建
   ...
   - 进程创建的起因
     ...
     - 作业调度
     - 提供服务
     - 应用请求：Unix 的 fork()，Win 的 CreatProcess()
   - 进程创建的步骤
     - 1）申请空白 PCB
     - 2. 为新进程分配资源
     - 3. 初始化进程控制块
...
3. 在一个单处理系统中存在 5 个进程，处于就绪队列中的就绪进程数最多为` `。
4. 若记录型信号量的 wait(s)和 signal(s)操作的信号量 s 初值为 2，当前值为-1，则在 s 的阻塞队列中处于阻塞态的进程数为` `。
